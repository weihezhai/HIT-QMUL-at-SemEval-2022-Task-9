                ...
                if 'ingredients' in item_pre:
                    ...
                    while i < docs_lines and all_data[i] and "sent_id" not in all_data[i] and 'newdoc id' not in \
                        ...
                        if len(tags) != 1:
                            ...
                            tagcols['participant'].append(tags[5])
                    ...
                    tagcols['participant'] = part_to_text(tagcols['participant'],
...
def mergeTextTag(columns: tuple, jsonFile_path):
    ...
    for doc_id, each_doc in file.items():
        ...
        for _ in texttag_list:
            ...
            newtag = []
            entitylist = ['O', 'B-EXPLICITINGREDIENT','B-HABITAT', 'B-IMPLICITINGREDIENT','_','I-EXPLICITINGREDIENT','I-IMPLICITINGREDIENT','I-HABITAT']
            for idx, _ in enumerate(zip(*unested)):  # add tag name and '#' for not meaningful tags
                if not all(l == '_' for l in _):
                    hashpadlist = ['[BOH] ' + columns[i] + ' : ' + t + ' [EOH] ' \
                                   for i, t in enumerate(_) if t not in entitylist]
...
# The second parameter indicates the tags you need ,like "'hidden','participant','entity'"
