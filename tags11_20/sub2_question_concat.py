        ...
        if "|" in tags:
            ...
            for tag in tags:
                if 'habitat' in tag or 'tool' in tag:
                    ...
                    # 这意味着对于同时有"Drop=mixture.2.2.10|Tool=spatula.2.3.1|Habitat=bowl.2.2.3",只会提取第一个
                    # 后续再考虑分开的
            ...
            if 'habitat' in tags or 'tool' in tags:
                ...
                if '_' in tags:  # medium_oven_-_proof_skillet
                    tags = tags.split("_")
                    tags = " ".join(tags)
                return tags
            else:
...
class myDataset(Dataset):  # 需要继承data.Dataset
    ...
    def __getitem__(self, index):
        ...
        input_ids, token_type_ids, attention_mask = encode['input_ids'], encode['token_type_ids'], encode[
            'attention_mask']
        # 获取起始位置
        start, end = self.start_end(a, q, tag + " " + text)
