...
def train(model,tokenizer, train_dataloader, testdataloader,device,fold,epoch=3):
    ...
    for epoch in range(epoch):
        ...
        print("{},Train_acc:{} Train_acc2:{} Train_loss:{}-----Val_acc:{} Val_acc:{} Val_loss:{}".format(epoch,np.mean(train_acc),np.mean(train_acc2),np.mean(train_loss),
                                                                                                         np.mean(test_acc),np.mean(test_acc2),np.mean(test_loss)))
